...
/// A participant's vote on a launcher image hash, including the action (add or remove).
#[near(serializers=[borsh, json])]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LauncherVote {
    pub action: LauncherVoteAction,
}
...
pub struct LauncherHashVotes {
    pub vote_by_account: BTreeMap<AuthenticatedParticipantId, LauncherVote>,
}
...
impl LauncherHashVotes {
    ...
    pub fn vote(
        ...
        let vote = LauncherVote {
            action: action.clone(),
        };
        if self
            .vote_by_account
            .insert(participant.clone(), vote)
            .is_some()
        {
...
impl AllowedLauncherImages {
    ...
    pub fn add(
        ...
        let compose_hashes: Vec<LauncherDockerComposeHash> = current_mpc_image_hashes
            .iter()
            .map(|mpc_hash| get_docker_compose_hash(launcher_hash.clone(), mpc_hash.clone()))
            .collect();
    ...
    pub fn add_mpc_image_compose_hashes(&mut self, mpc_image_hash: &MpcDockerImageHash) {
        for entry in &mut self.entries {
            let compose_hash =
                get_docker_compose_hash(entry.launcher_hash.clone(), mpc_image_hash.clone());
...
mod tests {
    ...
    fn test_compose_hash_uses_both_hashes() {
        ...
        let compose_1 = get_docker_compose_hash(launcher_1, mpc_hash.clone());
