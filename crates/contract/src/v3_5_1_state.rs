...
fn migrate_allowed_docker_image_hashes(
    ...
    // The old format stored (image_hash, docker_compose_hash, added).
    // The new format stores (image_hash, added) — compose hashes are now in AllowedLauncherImages.
    // We need to reconstruct AllowedDockerImageHashes from the old entries.
    // Since AllowedDockerImageHashes is opaque, we use the raw borsh approach.
    ...
    // AllowedDockerImageHashes wraps Vec<AllowedMpcDockerImage> internally.
    // Serialize the new vec and deserialize into AllowedDockerImageHashes.
    let serialized = borsh::to_vec(&new_proposals).expect("borsh serialization should not fail");
    AllowedDockerImageHashes::try_from_slice(&serialized)
